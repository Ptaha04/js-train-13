...
function complexConvert(data) {
  ...
  jsonObj = JSON.parse(data, (key, value) => {
...
function manipulateUrl(url) {
  ...
  newObj.href = newUrl.href;
  // Змінюємо протокол URL на https.
  newObj.protocol = newUrl.protocol = "https";
  // Змінюємо хост URL на 'newhost.com'.
  newObj.host = newUrl.host = "newhost.com";
  newObj.pathname = newUrl.pathname;
  newObj.search = newUrl.search;

  newObj.params = Array.from(newUrl.searchParams.entries());
...
function searchParamsURL(url) {
  ...
  const searchParam = newUrl.searchParams.entries();
...
function manipulateSearchParams(paramsObj, newUrl) {
  ...
  //   const keyObj = Object.keys(paramsObj); ?????????????????
...
function createURLWithParams(params, url) {
  ...
  for (const prop in Object.entries(params)) {
    // urlObj.searchParams.set(prop[0], prop[1]);
    urlObj.searchParams.set(prop, Object.entries(params)[prop]);
  }
...
function modifyURLParameters(url, params) {
  ...
  for (const prop in params) {
    newUrl.searchParams.set(prop, params[prop]);
  }
...
function checkURLParameters(url, params) {
  ...
  for (const elem of params) {
    if (newUrl.searchParams.has(elem)) {
      result[elem] = newUrl.searchParams.has(elem);
    } else {
      result[elem] = newUrl.searchParams.has(elem);
    }
  }
...
function getUrlData(url) {
  ...
  urlData.origin = newUrl.origin;
  urlData.hostname = newUrl.hostname;
  urlData.port = newUrl.port;
  urlData.username = newUrl.username;
  urlData.password = newUrl.password;
  return urlData;
}
...
function sortUrlParams(url) {
  ...
  const search = newUrl.searchParams.entries();
...
function getURLValues(url) {
  ...
  const searchArr = Array.from(newUrl.searchParams.entries());
...
function getUrlKeys(url) {
  ...
  const paramsArr = Array.from(newUrl.searchParams.keys());
